...
class Flow {
    constructor(datapack) {
        ...
        this.block = (coords, block) => ({
            _toMinecraftCondition: () => ({ value: ['if', 'block', index_1.coordinatesParser(coords), block] }),
        });
        /**
         ...
         */
        this.blocks = (start, end, destination, scanMode) => ({
            _toMinecraftCondition: () => ({ value: ['if', 'blocks', index_1.coordinatesParser(start), index_1.coordinatesParser(end), index_1.coordinatesParser(destination), scanMode] }),
        });
        /**
        ...
        this._while = (condition, callback, type) => {
            ...
            return {
                then: (onfulfilled) => {
                    promise.then(() => {
                        this.datapack.createEnterChildFunction(ASYNC_CALLBACK_NAME);
                        onfulfilled === null || onfulfilled === void 0 ? void 0 : onfulfilled();
                    });
                },
            };
        };
        ...
        this.binaryFor = (from, to, callback, maximum = 128) => {
            ...
            const realStart = from instanceof PlayerScore_1.PlayerScore ? from : this.datapack.Variable(from);
            const realEnd = to instanceof PlayerScore_1.PlayerScore ? to : this.datapack.Variable(to);
        ...
        this.forRange = (from, to, callback) => {
            const scoreTracker = from instanceof PlayerScore_1.PlayerScore ? from : this.datapack.Variable(from);
        ...
        // eslint-disable-next-line no-shadow
        callback) => {
            const realScore = score instanceof PlayerScore_1.PlayerScore ? score : this.datapack.Variable(score);
