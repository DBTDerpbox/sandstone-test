...
const index_3 = require("../variables/index");
...
class Datapack {
    constructor(packUid, namespace) {
        ...
        this.createObjective = (name, criteria = 'dummy', display) => {
            if (name.length > 16) {
                throw new Error(`Objectives cannot have names with more than 16 characters. Got ${name.length} with objective "${name}".`);
            }
            const objective = index_3.Objective(this.commandsRoot, name, criteria, display);
            this.registerNewObjective(objective);
            return objective;
        };
        /**
        ...
        this.Variable = (initialValue, name) => {
            ...
            const anonymousScore = score.ScoreHolder(`${name !== null && name !== void 0 ? name : 'anon'}_${datapack.packUid}_${id}`);
        ...
        this.Selector = index_3.SelectorCreator.bind(this);
        /** A BasePath changes the base namespace & directory of nested resources. */
        this.BasePath = (basePath) => new BasePath_1.BasePathClass(this, basePath);
        ...
        this.save = async (name, options) => {
            var _a;
            console.log(chalk_1.default `⌛ {gray Starting compilation...}`);
        ...
        this.flow = new index_2.Flow(this);
        this.initCommands = [];
    }
