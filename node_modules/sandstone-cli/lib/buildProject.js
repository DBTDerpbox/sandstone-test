...
async function _buildProject(options, { absProjectFolder, rootFolder, sandstoneConfigFolder }) {
    ...
    const { getDestinationPath } = require(path_1.default.join(sandstoneLocation, '_internals', 'datapack', 'saveDatapack'));
    ...
    for await (const filePath of walk(absProjectFolder)) {
        ...
        try {
            require(path_1.default.resolve(filePath));
        }
        catch (e) {
            logError(e);
            error = true;
    ...
    // Save the pack
    const { savePack } = require(path_1.default.join(sandstoneLocation, 'core'));
